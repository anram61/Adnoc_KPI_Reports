document.getElementById("report-form").addEventListener("submit", function(e) {
  ...
  // Calculate KPI (exclude financial pillar)
  ...
  // Save KPI score in localStorage per company
  ...
  // Update All Companies KPI List
  updateCompanyKPIList();

  // Generate report HTML with KPI bar
  const reportHTML = `
    <div style="font-family: Arial; background: #f7f9fc; padding: 20px; border-radius: 8px;">
      ...
      <div style="margin: 20px 0;">
        ...
        <div style="width: 100%; height: 20px; border-radius: 10px; overflow: hidden; background: #ddd;">
          <div style="width: ${kpiScore}%; height: 100%; 
            background: ${getKPIColor(kpiScore)};"></div>
        </div>
      </div>
  ...
  // Save report as HTML file in localStorage
...
// Load company KPI list when admin page loads

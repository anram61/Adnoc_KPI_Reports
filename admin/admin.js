...
function makeCompanyKpiMap(currentCompany, currentValue){
  ...
  const map = {};
  companies.forEach(c=>{
    map[c] = (c===currentCompany)?currentValue:(kpis[c]??0);
  });
  return map;
}
...
function renderTrendChart(canvasId, monthsStr, values){
  ...
  if(!ctx) return;
  const months = monthsStr?monthsStr.split(',').map(m=>m.trim()):["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"];
  const data = values.length===months.length?values:Array(months.length).fill(values[0]);
  const projection = data.map((v,i)=>Math.min(5,v + i*0.05));
  ...
  new Chart(ctx,{
    ...
    data:{
      labels:months,
      datasets:[
        {label:'Actual', data:data, borderWidth:3, borderColor:'#1d7ed6', tension:0.35, spanGaps:true, fill:false},
        {label:'Projection', data:projection, borderWidth:2, borderDash:[6,6], borderColor:'#ff7b23', tension:0.35, spanGaps:true, fill:false}
      ]
    },
    options:{
      responsive:true,
...
generateBtn.addEventListener('click', ()=>{
  ...
  const graphMonths = monthsInputEl.value.trim();
  ...
  renderTrendChart('trendChart', graphMonths || '', Array((graphMonths?graphMonths.split(',').length:12)).fill(overall));
...
saveHomeBtn.addEventListener('click', ()=>{
  alert('Report saved successfully and will appear in the viewer.');
